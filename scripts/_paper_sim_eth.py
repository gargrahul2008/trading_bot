...
print("loading ETH Jan1->Jul28 ...", flush=True)
prov = MarketDataProvider(repo_root=ROOT, verbose=False)
df = prov.load('ETHUSDT','crypto','2026-01-01','2026-07-28').copy()
...
cfg = {"symbols":["ETHUSDT"], "trade_value_usd":5000, "entry_next_bar_open":True,
       ...
       "telegram":{"enabled":False}, "output_dir":"artifacts/_papersim",
...
for k,d in enumerate(ds):
    ...
    for i in range(len(day)):
        bot.on_bar('ETHUSDT', day.iloc[:i+1])
    if k % 20 == 0: print(f"  day {k}/{len(ds)} {d}", flush=True)
ptr = bot.paper_trades['ETHUSDT']
...
r = e.run(df, BacktestConfig(output_dir="artifacts/_papersim_bt",instrument_specs={'ETHUSDT':InstrumentSpec(quantity_mode="units",lot_size=1,instrument_type="equity")},
...
Path("artifacts/_papersim").mkdir(parents=True, exist_ok=True)
json.dump(out, open("artifacts/_papersim/result.json","w"), indent=2)
