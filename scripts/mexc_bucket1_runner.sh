#!/bin/bash
# mexc_bucket1_runner.sh — Run MEXC Bucket 1 (tight 0.4% grid) with auto-restart and Telegram alerts.
#
# Starts the bot in an infinite loop. On any exit, sends Telegram alert and restarts.
#
# Graceful stop: kill $(pgrep -f mexc_bucket1_runner.sh)

set -euo pipefail
cd /root/trading_bot

BUCKET="bucket1"
CONFIG="strategies/pct_ladder/config.mexc.bucket1.json"
...
stop_cleanly() {
    ...
    send_telegram "⏹ *Bucket1 stopped* (manual) — $(date '+%Y-%m-%d %H:%M:%S IST' --date='TZ=\"Asia/Kolkata\"')" || true
    exit 0
}

trap stop_cleanly SIGTERM SIGINT

LOCKFILE="/tmp/mexc_bucket1_runner.lock"
exec 9>"$LOCKFILE"
if ! flock -n 9; then
...
send_telegram "🚀 *Bucket1 started* — $(date '+%Y-%m-%d %H:%M:%S IST' --date='TZ=\"Asia/Kolkata\"')" || true
...
while [ "$STOP_REQUESTED" -eq 0 ]; do
    ...
    MSG="⚠️ *Bucket1 stopped unexpectedly* (exit=${EXIT_CODE})
Start: ${START_TIME}
Stop:  ${END_TIME}
Run #${ATTEMPT}
